 ...
 * @LastEditors: wkiwi
 * @LastEditTime: 2019-11-05 21:16:27
 */
import qiniuUploader from './qiniuUploader.js';
import {AppOtherService} from '../../services/api.service';
class Base {
  constructor() {
    "use strict";
    this.__init();
      ...
      shareContent(title, path, imageUrl, openUserName) {
        ...
        if (openUserName) {
          ...
          if (title) {
            ...
            newTitle = username + "来【打卡】与鱼塘的好友一起交流吧！";
          }
        } else {
					console.log(111)
          if (title) {
            ...
            newTitle = "来【打卡】与鱼塘的好友一起交流吧！";
          }
        }
        var raw = {
          title: newTitle,
          path: path ? path : "/pages/home/home",
          imageUrl: imageUrl
            ? imageUrl
      ...
      setClipboardData(str) {
        uni.setClipboardData({
          ...
          fail: function() {
			...
			/**
			 * 上传多张图片
			 * @param {Object} params [params.imgList为调用相册返回的临时文件列表]
			 * @return {*}
			 */
			imgUpload(params){
				let  imgList = params.imgList
				let _this =this
				let auth =uni.getStorageSync('auth', auth);
				let state = 0;//计数器
				let imgSuccessUrlList = []//上传成功列表
				return new Promise(function(resolve, reject) {
					AppOtherService.getQnyToken({//获取到七牛token与主域名
						uid: auth.u_id
					})
					.then(res => {
						if(res.data.code !=0){
						...
						let uptoken = res.data.result.string
						let domain = res.data.result.domain
						new Promise(function(resolve, reject) {
							for (var i = 0; i < imgList.length; i++) {
								let imgItem = imgList[i]
								qiniuUploader.upload(imgItem, (imgResult) => {
										state++;
										// '返回需要上传给服务端的图片url', imgResult.imageURL;
										var image = {};
										let arr =[]
										image.qnyimg = 'https://' + imgResult.imageURL;
										image.orgimg = imgItem;
										image.key = imgResult.key;
										arr.push(image);
										imgSuccessUrlList = imgSuccessUrlList.concat(arr);
										if (state == imgList.length) {//上传完毕
											resolve(imgSuccessUrlList);
										}
									}, (error) => {
										reject('error');
										...
										domain: domain,
										uptoken: uptoken,
										key: auth.u_id + (new Date().getTime()) + (Math.ceil(Math.random() * 1000)) //生成随机图片名称:用户ID+时间戳+随机数
									},
									(progress) => {
						...
						.then(function(imgSuccessUrlList) {
							resolve(imgSuccessUrlList);
						})
					})
					.catch(err => {
